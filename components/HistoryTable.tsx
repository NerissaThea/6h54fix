  ...
  {isLoading ? (
    ...
    transactions.map((tx, index) => (
      <TableRow key={index} className="bg-white text-black">
        ...
        <TableCell>{tx.block || "N/A"}</TableCell>
        <TableCell>{tx.timestamp ? getRelativeTime(tx.timestamp) : "N/A"}</TableCell>
        <TableCell>{tx.from ? truncateAddress(tx.from) : "N/A"}</TableCell>
        <TableCell>{tx.to ? truncateAddress(tx.to) : "N/A"}</TableCell>
        <TableCell>{tx.amount ? `${tx.amount.toFixed(6)} ETH` : "0 ETH"}</TableCell>
        <TableCell>{tx.fee ? `${parseFloat(tx.fee).toFixed(6)} ETH` : "0 ETH"}</TableCell>
      </TableRow>
    ))
  ) : (
    <TableRow>
